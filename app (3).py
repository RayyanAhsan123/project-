...
def get_groq_key() -> str:
    """Read Groq API key from Streamlit secrets (set via Advanced Settings)."""
    try:
...
def get_groq_client():
    """Return a Groq client, or None if key missing."""
    key = get_groq_key()
    if not key:
        return None
...
with tabs[2]:
    ...
    if not st.session_state.model_trained:
        ...
        st.warning("⚠️  Add GROQ_API_KEY in **Settings → Secrets** (Advanced Settings).")
    else:
        df  = st.session_state.df
        rf  = st.session_state.rf
        sc  = st.session_state.scaler
